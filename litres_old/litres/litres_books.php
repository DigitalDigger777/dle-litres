...
ini_set('error_log', 'php_errors.log');
	...
	while ($row = mysql_fetch_array($result)){
			...
			*/
			...
			if ($row['local_book_id_litres_catalog'] == NULL){
				echo $q = "INSERT INTO dle_post SET
					...
					short_story = '" . mysql_real_escape_string($short_story) . "',
					full_story = '" . mysql_real_escape_string($full_story) . "',
					...
					symbol = '',
					tags = '',
					metatitle = '" . mysql_real_escape_string($title) . "'
					";
				mysql_query($q);
				...
				echo $q = "UPDATE dle_post SET
					...
					full_story = '" . mysql_real_escape_string($full_story) . "',
					xfields = '" . mysql_real_escape_string($xfields_str) . "',
					title = '" . mysql_real_escape_string($row['book_title']) . "',
					descr  = '',
					category = '" . $local_categories . "'
		...
		$res_extra = mysql_query($q);
		//вставляем данные только если новая книга и записи еще нет
		if (mysql_num_rows($res_extra) == 0){
			mysql_query("INSERT IGNORE INTO dle_post_extras (news_id,user_id) VALUES (". $local_book_id .",103342)");
		}
