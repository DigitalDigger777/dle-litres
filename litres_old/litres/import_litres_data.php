	...
	foreach ($types_array as $type){
		...
		foreach ($xml->updated_book as $value){
			...
			elseif ((($value->attributes()->must_import == 1)) || $type == 12){
				...
				$publisher = $value->publish_info->publisher;
				...
				$q = "INSERT INTO `litres_data" . ($type == 12 ? '_paper' : '') . "` SET 
						hub_id = ".$hub_id.",
						litres_id = ".$litres_id.",
						litres_url = '" . mysql_real_escape_string($book_url) . "',
						" . ($hub_author_id !='' ? "hub_author_id = " . $hub_author_id . "," : "") . "
						litres_a_url = '" . mysql_real_escape_string($author_url) . "',
						global_book_id = '".$global_book_id."',
						has_trial = '".$has_trial."',
						author_name = '" . mysql_real_escape_string($author_name) . "',
						author_sname = '" . mysql_real_escape_string($author_sname) . "',
						litres_a_rod = '" . mysql_real_escape_string($author_rodit) . "',
						author_lvl = '" . $author_lvl . "'," .
						($second_author_name != '' ? "second_author_name = '" . mysql_real_escape_string($second_author_name) . "'," : "") . 
						($second_author_sname != '' ? "second_author_sname = '" . mysql_real_escape_string($second_author_sname) . "'," : "") . "
						book_title = '" . mysql_real_escape_string($book_title) . "',
						genre = '".$genre."',
						genre_names = '".$genre_names."',".
						//sequences = '".mysql_real_escape_string($sequences_names)."',
						"options = ".$options.",
						...
						cover_ext = '".$cover_ext."',
						contract_title = '" . mysql_real_escape_string($contract_title) . "',
